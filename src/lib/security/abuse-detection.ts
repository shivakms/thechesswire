...
async function checkRateLimit(ip: string, fingerprint: string, action: string): Promise<{exceeded: boolean}> {
  ...
  console.log(`Rate limiting check for ${action}:`, { ip, fingerprint });
...
async function checkAbuseHistory(ip: string, fingerprint: string): Promise<{previousAbuse: boolean; score: number}> {
  ...
  try {
    ...
    const row = result.rows[0] as { abuse_count: string; max_score: string };
    const abuseCount = parseInt(row.abuse_count);
...
async function logAbuseCheck(data: AbuseCheckData): Promise<void> {
  const db = await getDb();
